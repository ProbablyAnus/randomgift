...
2. Заполните `.env`:
   ...
   - `API_BASE_URL` — базовый URL API (например, `http://localhost:8080` для локальной разработки без reverse proxy).
...
- Архив неиспользуемых компонентов на переходный этап: `src/components/ui/_unused`.
